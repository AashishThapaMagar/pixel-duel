...
func run() -> void:
	...
	for i in 7:
		...
		check(visual.gait is Node2D and visual.gait.meshes.size() == 5, "Walking deforms the standing art's own legs and arms via a five-part mesh")
		for mesh in visual.gait.meshes:
			check(mesh.material is ShaderMaterial, "The gait mesh is keyed by the same magenta-backdrop shader as the sprite")
		for frame in 24:
