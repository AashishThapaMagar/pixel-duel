...
The implementation keeps WHO WON?, its original fighters, and its existing modes. The gold arcade publisher splash leads straight to the four-door Himalayan menu, with no character entrance animation. Arrow keys choose a mode; clicking a door or pressing Enter opens its double doors and fades into character selection. Repeated input is locked during the transition. Settings and other secondary actions remain available below the doors. Settings, arena previews and move guidance remain accessible.
...
Door validation: `tests/door_menu_test.gd` checks immediate input readiness, arrow navigation, modal isolation, repeated-click protection and the animated transition into character selection. The arcade, UI flow and story tests include the new transition duration.

Mode interiors now use separate compositions while retaining the four-door home menu. Local Versus has paired human fighter panels; Computer Challenge adds a CPU rival picker and live difficulty selector; Arcade shows a hero and the rival ladder; Story places the chapter introduction beside the protagonist. Each mode has its own scenery, accent and start action. CPU rivals remain ready after selection, and campaign confirmation keeps focus on the human fighter. The bottom menu options retain their existing behavior.

`tests/menu_options_test.gd` checks mode presentation, CPU difficulty/readiness and protected campaign opponent selection; `--capture` saves a screenshot of each mode to `.godot/option-*.png`.
