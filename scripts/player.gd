...
##
## The body is an AnimatedSprite2D (assets/sprites/fighter/*.png, generated by
## assets/sprites/fighter/generate_fighter.py) with one animation per state —
## idle sway, walk cycle, jump, jab/hook/kick, guard stance, hit flinch, KO.
## The art is grayscale so it can be tinted per-player via `Visual.modulate`
## (blue for P1, red for P2) — one sheet, two fighters. Facing flips the
## sprite via `Visual.flip_h`.
##
...
# Picks the sprite animation for the current state/attack variant and plays
# it — cheap to call every physics frame since play() only restarts an
# animation that isn't already the one running.
